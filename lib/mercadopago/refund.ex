defmodule Mercadopago.Refund do
  ...
  @doc "Fetches a single refund of the given payment."
  @spec get(Client.t(), HTTP.id(), HTTP.id(), keyword()) :: HTTP.response()
  def get(%Client{} = client, payment_id, refund_id, opts \\ []) do
    HTTP.get(
      client,
      "/v1/payments/#{HTTP.encode_path_param(payment_id)}/refunds/#{HTTP.encode_path_param(refund_id)}",
      nil,
      opts
    )
  end
